...
public class SignUpGoal extends AppCompatActivity {
    ...
    public void signUpGoalSubmit() {
        ...
        if (errorMessage.isEmpty()) {
            ...
            /* 1: BRM */
            // Start calculation (Mifflin-St Jeor equation)
            double goalEnergyBMR = 0;
            if (stringUserGender.startsWith("m")) {
                // Male
                // BMR = (10 x kg body weight) + (6.25 x height in cm) - (5 x age) + 5
                goalEnergyBMR = (10 * doubleUserCurrentWeight) + (6.25 * doubleUserHeight) - (5 * intUserAge) + 5;
                //bmr = Math.round(bmr);

            } // if(stringUserGender.startsWith("m")){
            else {
                // Female
                // BMR = (10 x kg body weight) + (6.25 x height in cm) - (5 x age) - 161
                goalEnergyBMR = (10 * doubleUserCurrentWeight) + (6.25 * doubleUserHeight) - (5 * intUserAge) - 161;
                //bmr = Math.round(bmr);
            }
    ...
    public void mesurmentUsed() {
        ...
        if (mesurment.startsWith("m")) {
            ...
            // Kg to punds
            TextView textViewTargetMesurmentType = findViewById(R.id.textViewTargetMesurmentType);
            textViewTargetMesurmentType.setText("pounds");


            // Kg each week to pounds each week
            TextView textViewKgEachWeek = findViewById(R.id.textViewKgEachWeek);
